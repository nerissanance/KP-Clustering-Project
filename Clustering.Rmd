...
outcome <- data %>% select(id, target)
datanoout <- as.data.frame(data %>% select(-target))
class(datanoout)
...
Choosing a distance calculation is very important, as it defines how the relationship between eelments is mesures, and will therefore characteristically affect the shape of clusters.

Some options for distance matrices include:
...
data_s <- scale(datanoout)
options(scipen=999)
apply(data_s,2, function(x){round(summary(x),2)})
...
(add here: repeat 1-2 of the analyses with the reduced dataset)

```{r}

umap_output <- umap(data)

layout <- as.data.frame(cbind(umap_output$layout, umap_output$data))
...
#look at this by k-means cluster instead
layout2 <- cbind(layout, km$cluster)
names(layout2)
ggplot(data=layout2,aes(V1,V2, color=as.factor(km$cluster))) + geom_point() +labs(x="UMAP1", y="UMAP2",title="Scatterplot of UMAP embeddings of heart disease data")+theme_classic()+scale_fill_continuous(name = "Presence of heart disease")
